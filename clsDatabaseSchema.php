...
class clsDatabaseSchema
{
	...
	public function ListAttributes( $db )
	{
...
(SELECT count(*)
	  ...
	  WHERE tc.constraint_type = 'FOREIGN KEY'
	    AND tc.table_schema = '$db' and u.table_name=c.table_name
	  ORDER BY u.table_schema,u.table_name,u.column_name) as foreign_key
FROM information_schema.tables t
JOIN information_schema.columns c ON t.table_schema = c.table_schema AND t.table_name = c.table_name
WHERE t.table_schema = '$db'
ORDER BY  foreign_key DESC, c.table_name, c.column_name";
		    ...
		    if( !is_bool( $res )) 
		    {
		    		$i=0;
				while ($row = mysql_fetch_row($res)) { 
					$ret[$i][0] = $row[0]; //column
				   	$ret[$i][1] =  $row[1];  //data type
				   	$ret[$i][2] =  $row[2]; // comment
				   	$ret[$i][3] = ""; //colunm type
				   	$ret[$i][4] = $row[3]; //table name
					$ret[$i][5] = $row[4]; //has foreign keys				
					$i = $i + 1 ; 
	...
	public function ListDimensions( $db, $table )
	{
	  ...
	  USING( constraint_schema, constraint_name )
	  WHERE c.constraint_type = 'FOREIGN KEY'
	    AND c.table_schema = '$db' and u.table_name='$table'
	  ORDER BY u.table_schema,u.table_name,u.column_name";
		    ...
		    if( !is_bool( $res )) 
		    {
		    		$i=0;
				while ($row = mysql_fetch_row($res)) { 
					$ret[$i][0] = $row[0]; //table schema
				   	$ret[$i][1] = $row[1];  //table
				   	$ret[$i][2] = $row[2]; // key
				   	$ret[$i][3] = $row[3]; // referenced schema
				   	$ret[$i][4] = $row[4]; //referenced table
					$ret[$i][5] = $row[5]; //referenced key
					$ret[$i][6] = $this->ListColumns( $db, $row[4]);			
					$i = $i + 1 ; 
	...
	public function ListPreview( $db)
	{
		$table="dado_ficha";
		$measure="idficha";
		$fk="cobribge";	 
		//$dimension=["table", "pk_table", "attribute"];	 	
		$dimension=array("dado_localbrasilibge", "cobribge", "nobribge");	 		 	
		$ret = array();
		$query  = "SELECT 
			$table.$measure
			,dado_localbrasilibge.nobribge
			FROM $table
			JOIN ".$dimension[0]." ON ".$dimension[0].".".$dimension[1]."=$table.$fk";
		    ...
		    if( !is_bool( $res )) 
		    {
				...
				while ($row = mysql_fetch_row($res)) { 
					$ret[$i][0] = $row[0]; //column
				   	$ret[$i][1] =  $row[1];  //data type
				   	$ret[$i][2] =  $row[2]; // comment
				   	$ret[$i][3] = $row[3]; //colunm type
				   	$ret[$i][4] = $row[4]; //table name				
					$i = $i + 1 ; 
